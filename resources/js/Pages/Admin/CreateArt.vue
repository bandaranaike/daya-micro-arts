    ...
    <authenticated-layout>
        ...
        <div class="w-full">
            <form class="w-full mx-auto max-w-3xl">
                <h1 class="text-2xl py-6">Create new art</h1>
...
function removeImage(art) {
    axios.patch(`/art/remove-image/${art.uuid}`).then(response => {
        art.image = null;
        imageDeleteMessage.value = response.data;
    })
}
...
function saveArt() {
    ...
    const savingUrl = isEditing.value ? '/art/update/' + art.uuid : '/art/create'
